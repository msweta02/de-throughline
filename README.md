...
| [Architecture](#architecture) | the pieces, the three files, and where the data goes |
...
```
   ...
   ┌───────────────────────────┐         ┌───────────────────────────┐
   │ your task                 │         │ api.py    FastAPI, mounted│
   │ ┌───────────────────────┐ │         │           in-process by   │
   │ │ @throughline.trace    │ │         │           fastapi_apps    │
   │ │  1 snapshot the input │ │         │ views.py  pages as HTML   │
   │ │  2 run the real body  │ │         │ grid.py   grid + strip    │
   │ │  3 snapshot the output│ │         │ replay.py re-execute one  │
   │ └───────────────────────┘ │         │           record          │
   └──────┬─────────────┬──────┘         └─────────────┬─────────────┘
          │             │                              │
 the task │             │ Throughline            reads │
  writes  │             │ writes                       │
          ▼             ▼                              ▼
   ┌──────────────┐   ┌──────────────────────────────────────────┐
   │ warehouse    │   │ throughline.duckdb                       │
   │ .duckdb      │   │   capture.captures  one row per field    │
   │ wh.orders …  │   │   capture.replays   one row per attempt  │
   └──────────────┘   └──────────────────────────────────────────┘
          ▲
          │ read-write on a normal run. During a replay it is attached
