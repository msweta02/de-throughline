...
* ``@throughline.trace`` sits above each step function, *below* ``@task`` — see the
  note in ``throughline/tracing.py`` for why that order and not the other one.
* ``params.throughline_scope`` renders to ``true`` on a normal run and to a
  predicate like ``order_id = 88231`` during a scoped replay. Only the task
  that reads a source table needs it.

Nothing else about these tasks was written for Throughline.
"""
...
def orders_enrichment() -> None:
    ...
    # outermost decorator, which is the order Throughline requires.
    ...
    extracted = extract(scope="{{ params.throughline_scope }}")
